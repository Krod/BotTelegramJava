	...
	public void run() throws Exception {
		...
		while (true) {
			...
			for (Update update : updates) {
				...
				System.out.println("Localização: " + "Latidude = " + locais.get(message.from().id()).latitude() + ", Longiture = " + locais.get(message.from().id()).longitude());
				...
				if (texto.equals("/ajuda")) {
					...
					sendResponse = bot.execute(new SendMessage(message.chat().id(), http.sendGet(texto).toString()));
